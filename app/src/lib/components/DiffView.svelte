<script>
  import { rawText, transformedText } from '$lib/stores.js';
  import { diffWords } from 'diff';

  $: parts = ($rawText && $transformedText)
    ? diffWords($rawText, $transformedText)
    : [];
...
<div class="diff-pane">
  <div class="diff-header">
    <div class="diff-title">
      ...
      <span class="side new">Transformed</span>
    </div>
    {#if parts.length > 0}
      <div class="stats">
  ...
  {:else}
    <div class="empty">
      ...
      <p class="hint">Record → transcribe → transform, then come back here.</p>
    </div>
  {/if}
</div>
